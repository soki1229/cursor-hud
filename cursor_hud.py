...
# ══════════════════════════════════════════════════════════════
#  EXE-SAFE PATHS
#  EXE: sys.executable 옆 디렉터리 / py: 스크립트 옆 디렉터리
# ══════════════════════════════════════════════════════════════
def _app_dir() -> Path:
...
def c(key: str) -> QColor:
    v = _THEME.get(key)
    if v is None or isinstance(v, str): return QColor()   # scrollbar 등 string 키 방어
...
def remain_color(remain_pct: float) -> QColor:
    """잔여량 기준 — 신호등 3구간 (테마 accent와 무관하게 의미 전달).
     0~25%  : c_red    위험
    26~50%  : c_amber  주의
    51~100% : c_green  여유
    """
...
class ArcGauge(QWidget):
    ...
    def paintEvent(self, _):
        ...
        # ── 빈 트랙: 반원 도넛 path 클리핑 후 사선 ──────────
        # 도넛 = outer 반원(왼→오) + inner 반원(오→왼)
        outer_r = r + sw // 2
        inner_r = max(1, r - sw // 2)
        track = QPainterPath()
        # outer 반원: 180° → 0° (반시계 방향, Qt에서 양수 span = 반시계)
        track.moveTo(cx - outer_r, cy)
        track.arcTo(QRectF(cx - outer_r, cy - outer_r, outer_r * 2, outer_r * 2),
                    180, -180)  # 180°에서 시작, -180° span = 시계방향으로 반원
        # inner 반원: 0° → 180° (반대방향으로 돌아와서 닫힘)
...
class CreditsPage(QWidget):
    ...
    def refresh_theme(self):
        """테마 변경 시 생성 시점에 고정된 색상들을 전부 재적용."""
        # 에러/힌트
        ...
        set_lbl_color(self._hdr_od,       c("c_amber"))
        set_lbl_color(self._hdr_rates,    c("accent2"))
        # 모든 kv_row: label(t_muted) + value(t_bright) 초기색 재적용
        # (update_data가 있으면 덮어쓰므로 안전)
...
class SettingsPage(QWidget):
    ...
    def _build(self):
        ...
        # Visibility toggles — 라디오 스타일 (ON / OFF)
